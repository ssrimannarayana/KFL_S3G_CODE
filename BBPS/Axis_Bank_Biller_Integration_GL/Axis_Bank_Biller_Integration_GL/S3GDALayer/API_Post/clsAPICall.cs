...
namespace S3GDALayer.API_Post
{
    public class clsAPIpost
    {
        ...
        public int FunPubPostReceiptDetails(int Option, int Request_ID, string strLoan_Numer, decimal amountPaid, string transactionId, string strpaymentMode, string paymentDate, out string strErroStatus)
        {
            strErroStatus = string.Empty;
            try
            {
                DbCommand command = db.GetStoredProcCommand("S3G_BBPS_POST_ACCOUNT_INFO_INS");
                db.AddInParameter(command, "@Option", DbType.Int32, Option);
                db.AddInParameter(command, "@Request_ID", DbType.Int32, Request_ID);
